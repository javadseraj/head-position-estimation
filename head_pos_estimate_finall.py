...
#Created on Thu Sep 10 21:22:11 2020
...
import cv2
import sys
from deepgaze.deepgaze.haar_cascade import haarCascade
from deepgaze.deepgaze.face_landmark_detection import faceLandmarkDetection
from math import pi,atan2,asin , degrees
import math 
import numpy as np
#If True enables the verbose mode
DEBUG = True 
image_adress = "C:\\Users\\User\\data\\head_pos_estimation_webcam"
image_name  = "\\face_shot-0"
image = image_adress+ image_name+".png"
...
def main():
    ...
    if(video_capture.isOpened() == False):
        ...
        print("The image source has been opened correctly...")
    ...
    my_cascade = haarCascade("..\\head_pos_estimation_webcam\\deepgaze\\etc\\xml\\haarcascade_frontalface_alt.xml", "..\\head_pos_estimation_webcam\\deepgaze\\etc\\xml\\haarcascade_profileface.xml")
    #TODO If missing, example file can be retrieved from http://dlib.net/files/shape_predictor_68_face_landmarks.dat.bz2
    my_detector = faceLandmarkDetection('..\\head_pos_estimation_webcam\\shape_predictor_68_face_landmarks.dat\\shape_predictor_68_face_landmarks.dat')
    ...
    if(True):
        ...
        if(my_cascade.face_type > 0):
            ...
            if(my_cascade.face_type > 0):
                ...
                pitch = degrees(asin(math.sin(pitch)))
                print(pitch)
                roll = -degrees(asin(math.sin(roll)))
                if (roll>45):
                    roll= roll-45;
                if(roll <-50) :
                    roll= roll +45 
               ...
               # eul    = euler_angles_radians
                ...
                #pitch  = ((eul[0,0]+math.pi/2)*math.cos(eul[1,0]))
                #roll   = ( (-(math.pi/2)-eul[0,0])*math.sin(eul[1,0]) + eul[2,0] )
        ...
        if(DEBUG == True):
            ...
            cv2.putText(frame,"roll_angle : " + str(int(roll)), (text_x1+100,text_y1), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (10,100,159), 2);
            cv2.rectangle(frame, 
                         (roi_x1, roi_y1), 
                         (roi_x2, roi_y2), 
                         (0, 255, 255),
        ...
        cv2.imwrite( image_adress+image_name+"tested"+ ".png" , frame )
